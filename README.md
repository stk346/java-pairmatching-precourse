...
홀수인 경우 마지막 남은 크루는 마지막 페어에 포함시킨다.  
이미 페어로 만난적이 있는 크루끼리 다시 페어로 매칭 된다면 크루 목록의 순서를 다시 랜덤으로 섞어서 매칭을 시도3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러 메시지를 출력  

- 페어를 섞는 기능
  - 준비된 파일로부터 List<String> 형태로 불러와야 함
  - Randoms.shuffle() 메서드로 리스르를 섞음
  - 섞인 리스트에서 순서대로 두명씩 페어를 맺음
  - 페어로 만난적이 있는 크루들끼리 만나면 다시 섞어서 매칭하는 시도를 3번 함.
  - 3회까지 매칭이 되지 않거나 매칭을 할 수 없는 경우 에러메시지 출력

- 페어를 매칭하는 기능
  - 과정, 레벨, 미션을 보여줌
  - 과정, 레벨, 미션을 선택하면 페어매칭 결과를 보여주고 메인 화면으로 이동
  - 매칭 정보가 없으면 에러메시지 출력 ([ERROR] 매칭 이력이 없습니다.)
  - 매칭 정보가 있으면 다시 매칭하시겠습니까 메시지 출력 후 인풋에 따라 기능 구현

- 페어를 조회하는 기능
  - 과정, 레벨, 미션을 보여줌
  - 인풋에 따라 매칭 정보가 있으면 보여줌.
  - 매칭 정보가 없으면 에러메시지 출력
