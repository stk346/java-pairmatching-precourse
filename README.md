...
- 기능을 선택
  - 페어 매칭
    - 과정, 레벨, 미션을 입력 받음
    - 매칭을 수행한 후 결과 출력
    - 매칭 정보가 있다면 메시지 출력 후 네 | 아니오에 따라서 다시 매칭하거나 하지 않는 기능 구현
  - 매칭 조회
    - 과정, 레벨, 미션을 입력 받음
    - 이전에 매칭했던 결과를 출력
  - 매칭 초기회
...
- 미션
  - 레벨에 따라 각기 다른 미션이 존재
  - 레벨, 미션에 따라 중복되지 않는 페어 쌍을 만들어 내야 함
- 페어
  - 각 코스에 따른 파일을 읽어 리스트를 섞는 기능
  - 리스트를 페어로 만들어야 함
  - 홀수이면 마지막은 3명으로 만들어야 함
  - 레벨에 따라 겹치지 않게 페어 생성하는 기능
  - 3번 시도 후 실패하면 에러 출력